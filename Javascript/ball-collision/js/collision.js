...
function start() {
  ballNum = getRandomFromRange(30, 200);
...
function generateBalls(ballNum) {
  while (balls.length < ballNum) {
    ...
    for (let i = 0; i < balls.length; i++) {
      ...
      if (
        distance <
        (randomRadius + ball.radius) * (randomRadius + ball.radius)
      ) {
        overlapping = true;
        break;
      }
    ...
    if (!overlapping)
      balls.push(new Ball(randomRadius, color, randomX, randomY));
  }
...
function ballCollision(ball) {
  ...
  for (let i = 0; i < balls.length; i++) {
    ...
    if (
      distance <
      (otherBall.radius + ball.radius) * (otherBall.radius + ball.radius)
    ) {
