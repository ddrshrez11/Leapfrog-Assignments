...
function generateBalls(antNum){
    while (ants.length < antNum){
        ...
        for (let i = 0; i < ants.length; i++) {
            let ant = ants[i];
            distance = getDistance(randomX+randomRadius, ant.x + ant.radius, randomY + randomRadius, ant.y + ant.radius);
            // distance = getDistance(randomX, ant.x, randomY, ant.y);
            if (distance < (randomRadius+ant.radius)*(randomRadius+ant.radius)){
                overlapping = true;
                break;
            }
        };
        if (!overlapping)    ants.push(new Ball(randomRadius, color, randomX, randomY));        
    }
...
function antCollision(ant){
    ...
    for (let i = 0; i < ants.length; i++) {
        ...
        distance = getDistance(otherBall.x + otherBall.radius, ant.x + ant.radius, otherBall.y + otherBall.radius, ant.y + ant.radius);
        // distance = getDistance(otherBall.x, ant.x, otherBall.y, ant.y);
        // console.log(ant.x,otherBall.x);
        if (distance < (otherBall.radius+ant.radius)*(otherBall.radius+ant.radius)){
...
function handleClick(event) {
    ...
    for (let i = 0; i < ants.length; i++) {
        if(getDistance(pointerX,ants[i].x,pointerY,ants[i].y) <= ants[i].radius**2){
            killAnt(i);
            console.log('clicked')
        }
